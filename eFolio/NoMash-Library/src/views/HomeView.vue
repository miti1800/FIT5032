      ...
      <div class="col-md-8 offset-md-2">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-3">
            ...
            <div v-if="msg.reason" class="text-success">
                  {{ msg.reason }}
            </div>
          </div>
          <div class="mb-3">
...
const validateReason = (blur) => {
  if (formData.value.reason.length < 10) {
    if (blur) errors.value.reason = 'Reason must be atleast 10 characters'
  } else {
    errors.value.reason = null
  }

  if (formData.value.reason.toLocaleLowerCase().includes("friend")) {
    if (blur) msg.value.reason = 'Great to have a friend'
  }
  else {
    msg.value.reason = null
  }
}
</script>
